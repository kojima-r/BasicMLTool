...
def evaluate(test_y,pred_y,prob_y,args,result={}):
	if args.task=="binary":
		...
		result["auc"]=auc
		precision, recall, f1, support=sklearn.metrics.precision_recall_fscore_support(test_y,pred_y,pos_label=1,average="binary")
		...
		precision, recall, f1, support=sklearn.metrics.precision_recall_fscore_support(test_y,pred_y,average='macro',pos_label=1)
		result["precision"]=precision
		result["recall"]=precision
		result["f1"]=f1
		conf=sklearn.metrics.confusion_matrix(test_y, pred_y)
